...
def test_create_subscription_invalid_url(client):
...
def test_get_subscription_invalid_uuid(client):
    """Test GET with a non-UUID string."""
    r = client.get("/subscriptions/not-a-uuid")
    assert r.status_code == 422
...
def test_get_subscription_not_found(client):
    ...
    r = client.get(f"/subscriptions/{non_existent_id}")
    assert r.status_code == 404
    assert "Subscription not found" in r.json()["detail"]

def test_patch_subscription_not_found(client):
    """Test PATCH with a valid UUID that doesn't exist."""
    non_existent_id = uuid.uuid4()
    payload = {"target_url": "http://doesnt-matter.com"}
    r = client.patch(f"/subscriptions/{non_existent_id}", json=payload)
    assert r.status_code == 404
    assert "Subscription not found" in r.json()["detail"]
...
def test_delete_subscription_not_found(client):
    ...
    non_existent_id = uuid.uuid4()
    r = client.delete(f"/subscriptions/{non_existent_id}")
    assert r.status_code == 404
    assert "Subscription not found" in r.json()["detail"]

# --- Ingest Endpoint Errors ---

def test_ingest_sub_not_found(client):
    """Test POST /ingest with a valid UUID that doesn't exist."""
    non_existent_id = uuid.uuid4()
    payload = {"data": 123}
    r = client.post(f"/ingest/{non_existent_id}", json=payload)
    assert r.status_code == 404
    assert "Subscription not found" in r.json()["detail"]
...
def test_ingest_invalid_uuid(client):
    ...
    payload = {"data": 123}
    r = client.post("/ingest/not-a-uuid", json=payload)
    assert r.status_code == 422
    assert "valid uuid" in r.json()["detail"][0]["msg"].lower()


def test_ingest_invalid_json_body(client, redis_conn): # Use redis_conn fixture
